...
<div class="layui-layout-admin site-demo">
  <div class="layui-main">
      <div class="layui-form-item">
        ...
        <div>
          <input id="inp-username" username="username" value="<%=username%>" type="text" class="layui-input" readonly="value">
        </div>
      </div>
...
function getSelectData() {
    ...
    var relations = {
        sql_class: "wspcms_auth_assignment", //表名  
        sql_top: "", //取数据总条数 top 10  
        sql_colums: "id,item_name,user_id", //列名，用","隔开，如果全部获取，则填写"*"   
        sql_whereBy: "and user_id = '" + updateid + "'",
        sql_orderBy: "order by item_name asc"
    }
    var Datas = '';
...
function renderData(item) {
    ...
    for (var i = 0; i < item.length; i++) {
        ...
        htm += '<option data-id=' + item[i].id + ' value=' + item[i].item_name + '>' + item[i].item_name + '</option>';
    }
    $("#select2").append(htm);
    //显示可用路由
    getRouteData();
