...
Class EasyASP_Fun
	...
	Public Function IdCard(ByVal a)
		If Len(a) = 15 Then 
			...
			Dim m : m = 0
			Dim b : Set b = Easp.List.NewArray("7 9 10 5 8 4 2 1 6 3 7 9 10 5 8 4 2")
			For i = 0 To 16
				m = m + (CLng(Right(Left(a,i+1),1)) * CLng(b(i)))
			Next 
			Dim x : Set x = Easp.List.NewArray("1 0 X 9 8 7 6 5 4 3 2")
			IdCard = a & x(m Mod 11)
		Else 
			IdCard = a
		End If 
	...
	'例子：If fun.Header("http://www.jam1.cn,http://easp.cn,null") Then 输出接口内容
	Public Function Header(ByVal arr)
		Response.AddHeader "Access-Control-Allow-Origin","*"
		Header = False
		Dim a : Set a = Easp.List.New
		a.Separator = ","
		a.Data = Replace(arr," ","")
