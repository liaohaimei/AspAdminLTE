...
'## easp.aspjpeg.asp
'## ------------------------------------------------------------------------------
'## Feature  : EasyAsp AspJpeg Class 
'## Version  : v0.2
'## Author  : xuhuan(zhongjingjing[at]gmail.com)
'## Update Date : 2010/12/4 15:22:12
'## Description : 基于AspJpeg 2 的easp插件
'#################################################################################
Class EasyAsp_AspJpeg
 ...
 Private Sub Class_Initialize()
  ...
  Easp.Error(10001) = "服务器没有安装AspJpeg组件."
  Easp.Error(10002) = "来源路径错误或文件不存在."
  Easp.Error(10003) = "存储路径错误或路径不存在."
  Easp.Error(10004) = "水印图片路径错误或水印图片不存在."
  Easp.Error(10005) = "参数不能为空."
  Easp.Error(10006) = "不是Gif格式的图片."
 End Sub
 ...
 Public Property Let SourcePath(ByVal s)
  s_SourcePath = Easp.Fso.MapPath(s)
  if not Easp.Fso.IsExists(s_SourcePath) then
 ...
 Public Property Let ToPath(ByVal s)
  s_ToPath = Easp.Fso.MapPath(s)
  if not Easp.Fso.IsExists(s_ToPath) then
   Easp.Error.Raise 10002
  end if
 End Property
 ...
 Public Function [Open](ByVal s) 
  ...
  select case typename(s)
   case "String"
    t_SourcePath = Easp.Fso.MapPath(s)
 ...
 Public Function RandCode(ByVal r, ByVal s, ByVal t)
  if Easp.Has(r) then
   ...
   t_RandCode = Easp.RandStr("4:0123456789abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ")
  end if  
  Session("RandCode") = t_RandCode
  
  t_SourcePath = Easp.Fso.MapPath(s)
  t_ToPath = Easp.Fso.MapPath(t)
  
  if not Easp.Fso.IsExists(t_SourcePath) then
   Easp.Error.Raise 10002
  end if
  ...
  s_AspJpeg.Quality=Easp.ifHas(s_Quality,100) '设置加水印后图片的质量
  ...
  RandCode =  Easp.ifHas(t_ToPath,t_SourcePath)
 End Function
 ...
 Public Function Thumbnail(ByVal s, ByVal t, ByVal w, ByVal h, ByVal q, ByVal f)
  ...
  t_Quality = Easp.ifHas(q,s_Quality)
  t_Width = Easp.ifHas(w,s_Width)
  t_Height = Easp.ifHas(h,s_Height)
  t_Force = Easp.ifHas(f,s_Force)
  ...
  s_AspJpeg.Quality = Easp.ifHas(t_Quality , Easp.ifHas(s_Quality,100))
  s_AspJpeg.Sharpen 1,250
  
  if t_Force then  
   t_NewImage_Size = Easp.IIF(CurrentWidth > CurrentHeight, CurrentWidth, CurrentHeight)
   set t_AspJpeg = [New]()
 ...
 Public Function Merge(ByVal s,ByVal t,ByVal r, ByVal x, ByVal y)
  ...
  if not Easp.Fso.IsExists(t_ToPath) then
   Easp.Error.Raise 10003
  end if
  
  if not Easp.Has(r) then
   t_ResultPath = Easp.Fso.MapPath(r)
  else
  ...
  t_x = Easp.ifHas(x,(t_To_AspJpeg.Width - t_Source_AspJpeg.Width) / 2)
  t_y = Easp.ifHas(y,(t_To_AspJpeg.Height - t_Source_AspJpeg.Height) / 2)
 ...
 Public Function WaterMarkPosition(ByVal source_w,ByVal source_h,ByVal width,ByVal height,ByVal pos)
  ...
  select case pos '水印位置  
   ...
   case else   '随机位置   
    Randomize
    t_Position(0) = Easp.Rand(0,(source_w - width)) 
    Randomize
 ...
 Public Function WaterMarkFont(ByVal Str,ByVal BackgroundImage,ByVal Pos,ByVal Quality,ByVal Opacity,ByVal Rotation,ByVal Color,ByVal Family,ByVal Bold,ByVal FontSize)  
  t_SourcePath = Easp.Fso.MapPath(BackgroundImage)
  ...
  if Easp.Has(Rotation) then
   b_AspJpeg.Canvas.Font.Rotation = Rotation  '倾斜度
  end if  

  b_AspJpeg.Canvas.Font.Color = Easp.ifHas(Color,s_PenColor) '颜色 
  
  b_AspJpeg.Canvas.Font.Family = Easp.ifHas(Family,"Arial") '字体 宋体/黑体/楷体/隶书/
  
  b_AspJpeg.Canvas.Font.Bold = Easp.ifHas(Bold,Easp.ifHas(s_BrushSolid,False))     '是否加骈 true/  

  b_AspJpeg.Canvas.Font.Size = Easp.ifHas(FontSize,30)
  
  b_AspJpeg.Canvas.Font.Opacity = 1
  
  b_AspJpeg.Canvas.Font.Quality = Easp.ifHas(Quality,s_Quality)
  
  
  FontHeight = Round( ( Easp.ifHas(FontSize,30) / 2 ))
  FontWidth = Round( FontHeight * Len(Str))
'  FontHeight = Easp.ifHas(FontSize,30)
'  FontWidth = FontHeight * Len(Str)
  ...
  if Easp.Has(s_Font) and not Easp.Has(Family) then
  ...
  t_AspJpeg.Canvas.DrawImage 0, 0, b_AspJpeg , Easp.ifHas(Opacity ,Easp.ifHas(s_Opacity,100) ) / 100 , s_BackGroundColor 
 ...
 Public Function WaterMarkJpeg(ByVal s,ByVal t,ByVal Pos,ByVal Quality,ByVal Opacity)
  ...
  if not Easp.Fso.IsExists(t_SourcePath) then
   if not Easp.Fso.IsExists(s_WaterMarkPath) then
  ...
  t_To_AspJpeg.Quality  = Easp.ifHas(Quality , s_Quality) 
  
  
  if t_PNGOutput then
   t_To_AspJpeg.Canvas.DrawPNG t_WaterMarkPosition(0), t_WaterMarkPosition(1) , t_Source_AspJpeg , Easp.ifHas(Opacity ,Easp.ifHas(s_Opacity,100)) / 100,s_BackGroundColor
  else
   t_To_AspJpeg.Canvas.DrawImage t_WaterMarkPosition(0), t_WaterMarkPosition(1) , t_Source_AspJpeg , Easp.ifHas(Opacity ,Easp.ifHas(s_Opacity,100)) / 100,s_BackGroundColor
  end if
 ...
 Public Function WaterMark(ByVal s,ByVal t,ByVal Pos,ByVal Quality,ByVal Opacity)  
  ...
  if Easp.Fso.IsFile(t_SourcePath) then
 ...
 Public Function Crop(ByVal s,ByVal t,ByVal tx,ByVal ty,ByVal bx,ByVal by)
  t_SourcePath = Easp.Fso.MapPath(s)
  t_ToPath = Easp.Fso.MapPath(Easp.ifHas(t,s))
  if not Easp.Fso.IsExists(t_SourcePath) then
   Easp.Error.Raise 10002
  end if
 ...
 Public Function GifResize(ByVal s,ByVal t,ByVal w,ByVal h,ByVal a)
  ...
  if  Lcase(Easp.Fso.Extof(t_SourcePath)) <> ".gif" then
   Easp.Error.Raise 10006
  end if  
  set t_AspJpeg = [New]()
  set t_Gif = t_AspJpeg.Gif
  t_Gif.Open t_SourcePath
  if not Easp.Has(h) then
   t_Gif.Resize w 
  else
   t_Gif.Resize w , h , Easp.ifHas(a,0)
  end if
