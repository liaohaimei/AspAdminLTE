...
'##  easp.bs.asp
'##  ------------------------------------------------------------------------------
'##  Feature      :  EasyAsp Bootstrap 代码片段生成插件
'##  Version      :  1.0
'##  For EasyASP  :  3.0+
'##  Author       :  Jam(2239559[at]qq.com)
'##  Update Date  :  2016年7月5日 星期二
'#################################################################################
Class EasyASP_Bs
	'定义内部变量
	...
	End Property
	Public Function [New]()
		Set [New] = New EasyASP_Bs
	End Function
	'设置代码片段源文本
	Public Default Function Str(ByVal String)
		Set Str = New EasyASP_BsCode
		If Easp.Str.IsInList("btn,btna,dropdown,btnGroup,FormInline,Form",Easp.Str.GetName(String,":")) Then 
			Str.Code = Str.Tag(String,Easp.Str.GetValue(String, ":"))
		Else 
			Str.Code = String
		End If 
	End Function
...
Class EasyASP_BsCode
	...
	Public Default Property Get Code()
		s_source = Easp.Str.iReplace(s_source,"{col-l}","col-sm-3")
		s_source = Easp.Str.iReplace(s_source,"{col-r}","col-sm-9")
		Dim Match,Matches
		Set Matches = Easp.Str.Match(s_source, "{(.+?)}")
	...
	Public Function Tag(ByVal t, ByVal l)
		listType = l
		Dim q : Set q = New EasyASP_Str_StringBuilder
		Select Case Easp.Str.GetName(t, ":")
			Case "btn"
			...
			Case "FormInline"
				Select Case Easp.Str.GetValue(t, ":")
					Case "text"
	...
	Private Function S(ByRef String)
		Set S = New EasyASP_BsCode
		S.Code = String
	End Function
	'替换Str字符串
	Public Function Str(ByVal value)
		Set Str = S(Easp.Str.iReplace(s_source,"{str}",value))
	End Function
	'替换ID
	Public Function Id(ByVal value)
		Set Id = S(Easp.Str.iReplace(s_source,"{id}",value))
	End Function
	'替换Name
	Public Function Name(ByVal value)
		Set Name = S(Easp.Str.iReplace(s_source,"{name}",value))
	End Function
	'替换CSS
	Public Function Css(ByVal value)
		Set Css = S(Easp.Str.iReplace(s_source,"{css}",value))
	End Function
	'替换Type
	Public Function [Type](ByVal value)
		Set [Type] = S(Easp.Str.iReplace(s_source,"{type}",value))
	End Function
	'替换Label
	Public Function Label(ByVal value)
		Set Label = S(Easp.Str.iReplace(s_source,"{label}",value))
	End Function
	'替换Url
	Public Function Url(ByVal value)
		'If InStr(Value,"http") = 0 Then Value = "http://" & Value
		Set Url = S(Easp.Str.iReplace(s_source,"{url}",value))
	End Function
	'替换Val
	Public Function Val(ByVal value)
		Set Val = S(Easp.Str.iReplace(s_source,"{val}",value))
	End Function
	'替换Disabled
	Public Function Disabled()
		Set Disabled = S(Easp.Str.iReplace(s_source,"{disabled}","disabled"))
	End Function
	'替换HideLabel
	Public Function HideLabel()
		Set HideLabel = S(Easp.Str.iReplace(s_source,"{labelhide}","class=""sr-only"""))
	End Function
	'替换PlaceHolder
	Public Function PlaceHolder(ByVal value)
		Set PlaceHolder = S(Easp.Str.iReplace(s_source,"{ph}",value))
	End Function
	'替换Row
	Public Function Row(ByVal value)
		Set Row = S(Easp.Str.iReplace(s_source,"{row}",value))
	End Function
	'替换Size
	Public Function Size(ByVal value)
		Set Size = S(Easp.Str.iReplace(s_source,"{size}",value))
	End Function
	'替换Multiple
	Public Function Multiple()
		Set Multiple = S(Easp.Str.iReplace(s_source,"{multiple}","multiple"))
	End Function
	'替换其他
	Public Function F(ByVal value)
		Set F = S(Easp.Str.iReplace(s_source,"{"&Easp.Str.GetName(value,":")&"}",Easp.Str.GetValue(value,":")))
	End Function
	'替换List
	Public Function [Loop](ByVal Data,ByVal t)
		Dim d,i,li,b,C,rname : d = ""
		If Easp.isN(data) Then data = "[{}]"
		If TypeName(data) = "String" Then Set d = Easp.Json.Parse(data)
		If TypeName(data) = "EasyASP_Json_Array" Then Set d = Data
		Set li = Easp.Str.StringBuilder
		Select Case t
			Case "li"
			...
			Case "checkbox"
				For i = 0 To d.Length-1
					b="":If Easp.Has(d(i)("disabled")) And d(i)("disabled") Then b="disabled"
					c="":If Easp.Has(d(i)("type")) And d(i)("type")="inline" Then c=" checkbox-inline"
			...
			Case "radio"
				...
				For i = 0 To d.Length-1
					b="":If Easp.Has(d(i)("disabled")) And d(i)("disabled") Then b="disabled"
					c="":If Easp.Has(d(i)("type")) And d(i)("type")="inline" Then c=" radio-inline"
		...
		d = li.ToString
		Set li = Nothing
		Set [Loop] = S(Easp.Str.iReplace(s_source,"{loop}",d))
	End Function
End Class
%>