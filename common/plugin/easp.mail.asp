...
'## easp.mail.asp 
'## ------------------------------------------------------------------- 
'## Feature     :   EasyAsp Mail Class 
'## Version     :   v0.2 Alpha 
'## Author      :   xuhuan(xu.huan.web.master[at]gmail.com) 
'## Update Date :   2010/06/04  
'## Description :   EasyAsp Jmail邮件发送（插件） 
...
'##     Easp.Ext("mail").Init()  '初始化jmail对象，因为下面的设置都需要涉及到jmail对象，为了让下面的参数设置顺序可以随意排列必须先初始化 
'##     Easp.Ext("mail").AddRecipient "********@126.com"  '增加联系人 
'##     Easp.Ext("mail").AddRecipient "********@126.com;********@126.com"  '增加多个联系人 
'##     Easp.Ext("mail").AddRecipient Array("********@126.com")  '数组方式增加联系人，数组也可以不这样设置，只要是数组就可以 
'##     Easp.Ext("mail").AddRecipientBCC "********@126.com"  '增加密件收件人,非必须 
'##     Easp.Ext("mail").AddRecipientCC "********@126.com"  '增加邮件抄送者,非必须 
'##     Easp.Ext("mail").Smtp="smtp.126.com" '设置SMTP 
'##     Easp.Ext("mail").From="********@126.com" '发送人的邮箱 
'##     Easp.Ext("mail").FromName="虚幻" '发送人姓名 
'##     Easp.Ext("mail").MailServerUserName="********@126.com" '发送人邮箱用户名 
'##     Easp.Ext("mail").MailServerPassword="********" '发送人邮箱密码 
'##     Easp.Ext("mail").Subject="Subjectsmtp.126.com" '邮件主题 
'##     Easp.Ext("mail").Body="<font color='red'>Bodysmtp.126.com</font>" '邮件body内容 
'##     Easp.Ext("mail").AppendText "<font color='red'>AppendText.126.com</font>" '增加文本内容 
'##     Easp.Ext("mail").AddAttachmentIn("t.rar;t.asp") '增加多个嵌入式附件 
'##     t_file=Easp.Ext("mail").File("") '返回附件数组，非必须 
'##     Easp.WN t_file(0) '输出第一个附件ID 
'##     Easp.WN Easp.Ext("mail").File(1) '输出第二个附件ID，如果 
'##     Easp.Ext("mail").HTMLBody="<font color='red'>HTMLBodysmtp.126.com</font>" '邮件HTMLBODY内容 
'##     Easp.Ext("mail").AppendHTML Easp.Ext("mail").File(0) '往邮件内容中添加附件，有附件的邮件将以html格式发送 
'##     Easp.Ext("mail").AddAttachment("t.rar") '增加一个普通附件 
'##     Easp.WN Easp.Ext("mail").QuickSend() '快速发送，输出发送邮件数 
'##     Easp.WN Easp.Ext("mail").RecipientsCount() '输出收件人数量 
'##     '2.一般发送邮件方式，不需要上面的设置，只需要一个函数就可以了，邮件和附件参数不符都可以采用字符串或数组形式，字符串形式的话多个数据之间用 ; 进行分隔。 
'##     Easp.WN Easp.Ext("mail").Send("smtp.126.com","********@126.com","虚幻","********@126.com","********@126.com","********","Subjectsmtp.126.com","","<font color='red'>HTMLBodysmtp.126.com</font>",1,"","","") '发送邮件，输出发送邮件数 

'##     '该函数的参数如下，每个参数的具体含义可以详见该函数头部的注释Easp.Ext("mail").Send(Smtp,From,FromName,Email,MailServerUserName,MailServerPassword,Subject,Body,HTMLBody,Priority,Silent,tCharset,tContentType) 
...
Class EasyAsp_Mail 
 ...
 Private Sub Class_Initialize 
  ...
  t_HTMLBody        = "" 
  t_File         = "" 
  Easp.Error(10001)   = "您的服务器不支持 Jmail 组件." 
  Easp.Error(10002)   = "发送者地址不能为空." 
  Easp.Error(10003)   = "发送者姓名不能为空." 
  Easp.Error(10004)   = "优先级必须为1-5之间的数字." 
  Easp.Error(10005)   = "Jmail 对象未创建." 
  Easp.Error(10006)   = "邮件地址不正确." 
  Easp.Error(10007)   = "SMTP地址为空或不正确." 
  Easp.Error(10008)   = "没有任何收件人." 
  Easp.Error(20001)   = "身份验证的用户名不能为空." 
  Easp.Error(20002)   = "身份验证的密码不能为空." 
  Easp.Error(30001)   = "邮件发送失败." 
 End Sub 
 ' ============================================ 
 ' 类终结 
 ' ============================================ 
 ...
 Public Property Let ISOEncodeHeaders(ByVal p) 
  s_ISOEncodeHeaders=Easp.IIF(p,p,False) 
 End Property 
 ...
 Public Property Let [ContentType](ByVal p) 
  s_ContentType=Easp.Ifhas(p,"text/html") 
 End Property 
 ...
 Public Property Let Priority(ByVal p) 
  If Not Easp.Test(p,"int") or int(p)<1 or int(p)>5 Then 
   Easp.Error.Raise 10004 
  End If 
  s_Priority=p 
 End Property 
 ...
 Public Property Get [File](ByVal p) 
  ...
  If p < 0 Then 
   [File]=t_File 
  Else 
   [File]=t_File(Easp.IIF(p > Ubound(t_File),Ubound(t_File),p)) 
  End If 
 End Property 
 ...
 Public Sub AddRecipient(ByVal p) 
  ...
  For i=0 to UBound(t_p) 
   If Not Easp.Test(t_p(i),"email") Then 
    Easp.Error.Raise 10006 
   End If 
 ...
 Public Function Jmail() 
  If Not Easp.IsInstall("JMAIL.Message") then 
 ...
 Public Function QuickSend()   
  If Not Easp.IsInstall("JMAIL.Message") then 
   Easp.Error.Raise 10001 
  End If 
  If Not Easp.Has(s_Smtp) then 
   Easp.Error.Raise 10007 
  End If 
  If Not Easp.Has(s_From) then 
   Easp.Error.Raise 10002 
  End If 
  If Not Easp.Has(s_FromName) then 
   Easp.Error.Raise 10003 
  End If 
  If Not Easp.Has(s_MailServerUserName) then 
   Easp.Error.Raise 20001 
  End If 
  If Not Easp.Has(s_MailServerPassword) then 
   Easp.Error.Raise 20002 
  End If 
  If Not Easp.Has(t_Subject) Then 
   t_Subject="无主题." 
  End If 
  If RecipientsCount()=0 then    
   Easp.Error.Raise 10008 
  End If 
  If Not isobject(s_jmail) Then 
   Set s_jmail = Server.CreateObject("JMail.Message")   
  End If   
 ...
 Public Function Send(ByVal Smtp,ByVal From,ByVal FromName,ByVal Email,ByVal MailServerUserName,ByVal MailServerPassword,ByVal Subject,ByVal Body,ByVal HTMLBody,ByVal Priority,ByVal Silent,ByVal tCharset,ByVal tContentType) 
  ...
  If Not Easp.Has(Smtp) then 
   Easp.Error.Raise 10007 
  End If 
  If Not Easp.Has(From) then 
   Easp.Error.Raise 10002 
  End If 
  If Not Easp.Has(FromName) then 
   Easp.Error.Raise 10003 
  End If 
  If Not Easp.Has(Email) then 
   Easp.Error.Raise 10008 
  End If   
  t_Email=ToArray(Email)  
  For i=0 to UBound(t_Email) 
   If Not Easp.Test(t_Email(i),"email") Then 
    Easp.Error.Raise 10006 
   End If 
  ...
  If t_jmail.Recipients.count=0 then    
   Easp.Error.Raise 10008 
  End If 
  If Not Easp.Has(MailServerUserName) then 
   Easp.Error.Raise 20001 
  End If 
  If Not Easp.Has(MailServerPassword) then 
   Easp.Error.Raise 20002 
  End If 
  If Not Easp.Has(Subject) Then 
   Subject="无主题." 
  End If 
  If Not Easp.Has(Priority) Then 
   Priority = 1 
  End If 
  If Not Easp.Has(Silent) Then 
   Silent = True 
  End If 
  If Not Easp.Has(tCharset) Then 
   tCharset = "UTF-8" 
  End If 
  If Not Easp.Has(tContentType) Then 
