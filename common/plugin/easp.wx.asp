...
Class EasyAsp_WX
    ...
    Public Function Get_Access_Token()
        ...
        Easp.Cache(CacheName).Expires = 120
        If Not Easp.Cache(CacheName).Ready or Easp.IsN(Get_Access_Token) Then
            ...
            IF instr(s_result,"access_token")>0 Then
                Get_Access_Token = Easp.Str.GetName(Easp.Str.GetValue(s_result,":"""),""",")
                Easp.Cache(CacheName) = Get_Access_Token                
                Easp.Cache(CacheName).SaveAPP
            Else
                Get_Access_Token = ""
            End IF
        Else
            Get_Access_Token = Easp.Cache(CacheName)
        End IF
        IF Easp.IsN(Get_Access_Token) Then Easp.WE "Access_Token Error"
    End Function
    ...
    Public Function Get_jsapi_ticket()
        ...
        Easp.Cache(CacheName).Expires = 120
        If Not Easp.Cache(CacheName).Ready or Easp.IsN(Get_jsapi_ticket) Then
            '获取Access_Token
            ...
            IF instr(s_result,"ticket")>0 Then
                Get_jsapi_ticket = Easp.Str.GetName(Easp.Str.GetValue(s_result,"ticket"":"""),""",")
                Easp.Cache(CacheName) = Get_jsapi_ticket                
                Easp.Cache(CacheName).SaveAPP
            Else
                Get_jsapi_ticket = ""
            End IF
            ...
            'Set Json = Nothing
        Else
            Get_jsapi_ticket = Easp.Cache(CacheName)
        End IF
    End Function
    ...
    Public Function OAuth_Get_Access_Token(code)
        ...
        'IF instr(OAuth_Get_Access_Token,"errcode")>0 Then Easp.WE "读取信息失败！"
    End Function
    ...
    Private Function HttpSend(url,stype,s_data)
        ...
        IF Easp.Has(s_data) Then Http.Data=s_data
